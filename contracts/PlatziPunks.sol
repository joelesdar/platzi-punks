...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory) {
        ...
        return string(abi.encodePacked(params, "&topType=", getTopType(_dna))); // returning all params in another way to avoid the stack problem.
    }
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "PlatziPunks #',
                tokenId.toString(),
                '", "description": "Platzi Punks are randomized Avataaars stored on chain to teach DApp development on Platzi and be la mera demencia", "image": "',
                image,
                '"}'
            )
        );
